        ...
        handles.push(tokio::spawn(async move {
            ...
            if job.path.exists() {
                ...
                let verification_result = tokio::task::spawn_blocking(move || {
                    verify_hash(&path_clone, &hash_clone)
                }).await;

                match verification_result {
                    Ok(Ok(true)) => {
                        // 哈希匹配，跳过下载
                        println!("DEBUG: 文件已存在且哈希匹配，跳过下载: {}", job.path.display());
                    ...
                    _ => {
                        // 哈希不匹配、I/O错误或任务panic。
                        // 最安全的方式是删除损坏/不可读的文件并重新下载。
                        println!("DEBUG: 文件存在但哈希错误或不可读，重新下载: {}", job.path.display());
...
async fn download_chunk(
    ...
    if !verification_result {
        return Err(LauncherError::Custom(format!(
            "Hash mismatch for {}: expected {}, got something else",
            job.path.display(),
            job.hash
        )));
    }
...
fn verify_hash(path: &std::path::Path, expected_hash: &str) -> Result<bool, LauncherError> {
    if expected_hash.is_empty() {
        // 如果没有提供哈希，我们无法验证。假设它是好的。
        // 这是为那些不包含哈希的旧库格式准备的。
        return Ok(true);
    }
